# -*- coding: utf-8 -*-
"""
Created on Wed Jul 26 16:24:19 2023
...
#====================
custom_alpha=0.1 # default=0.0001
...
predictors = inputTable[colname].values
response = inputTable['RD(mm)'].values
response = np.where(response > 12.5, 12.5, response)
...
# hidden_layer_sizes = [(i,i,i) for i in range(10, 30, 10)]
hidden_layer_sizes = [(i,i,i) for i in range(10, 110, 10)]
...
#for rnd_st8 in random_states:
            ...
            X_train, X_test, y_train, y_test = train_test_split(predictors, response, test_size=0.1, random_state=random_states) # for HPT, tune random_state between (rnd_st8,500)
            ...
            validationPredictions = regressionNeuralNetwork.predict(predictors)
            validationPredictions = np.where(validationPredictions > 12.5, 12.5, validationPredictions)
                   
            # Perform cross-validation
            kf = KFold(n_splits=10, shuffle=True, random_state=random_states) # for HPT, tune random_state between (rnd_st8,500)
            ...
            trueResponse = np.where(response > 12.5, 12.5, response)
            predictedResponse = validationPredictions
            ...
            plt.scatter(trueResponse, predictedResponse, alpha=0.5, color='b')
            # plt.plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'k--', color='r', lw=1.5)
            plt.plot([0, predictedResponse.max()+5], [0, predictedResponse.max()+5], '--', color='r', lw=1.5)
            ...
            plt.axis([0, predictedResponse.max()+5, 0, predictedResponse.max()+5])
            #plt.grid(True)
            ...
            plt.ylim([0, total+5])  # Set y-axis range
            # Create labels for the legend
            # Check if there's more than one element in `counts`
            if len(counts) > 1:
                labels = [f"Pass:{counts[1]}({pass_percentage:.2f}%)", f"Fail:{counts[0]}({100-pass_percentage:.2f}%)"]
            else:
                if counts[0] == 0:
                    labels = ["Pass:0(0.00%)", f"Fail:{counts[0]}(100.00%)"]
                else:
                    labels = [f"Pass:{counts[0]}(100.00%)", "Fail:0(0.00%)"]
            ...
            print('========Pass-Fail=========')
            if len(counts) > 1:
                print(f"Pass:{round(counts[1], 2)}({pass_percentage:.2f}%)")
                print(f"Fail:{round(counts[0], 2)}({100-pass_percentage:.2f}%)")
            else:
                if counts[0] == 0:
                    print("Pass:0(0.00%)") 
                    print(f"Fail:{counts[0]}(100.00%)")
                else:
                    print(f"Pass:{counts[0]}(100.00%)")
                    print("Fail:0(0.00%)")
            print('==========================')
            print('///////////////////////////////////////')
            print('')
            
            output_df = pd.concat([output_df, pd.DataFrame([{'Output': 'Pass Count', 'Value': counts[1]}])], ignore_index=True)
            output_df = pd.concat([output_df, pd.DataFrame([{'Output': 'Fail Count', 'Value': counts[0]}])], ignore_index=True)
...
output_df.to_excel('RD_NN(I)_DIA_retain(3)_to_Excel_normal_output_iii.xlsx', index=False)
