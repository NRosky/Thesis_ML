...
random_states=42#0, 42, 50, 100, 200, 250, 500]
...
predictors = scaler_X.fit_transform(predictors)#.reshape(-1,1)).flatten()
...
#for rnd_st8 in random_states:
            ...
            plt.plot([0, predictedResponse.max()+0.1], [0, predictedResponse.max()+0.1], '--', color='r', lw=1.5)
            ...
            plt.axis([0, predictedResponse.max()+0.1, 0, predictedResponse.max()+0.1])
            #plt.grid(True)
...
output_df.to_excel('RD_NN(I)_DIA_retain(3)_to_Excel_normal_output_n100iii.xlsx', index=False)
